...
use crate::{
    ToolOperation,
    renderer::{build_path, fill_on_pixmap, stroke_on_pixmap},
};
use cosmic::iced::{Color, Point, Rectangle, Size, alignment::Horizontal, mouse};
...
impl TextPreview {
    ...
    pub fn sync_buffer_size(&mut self, scale: f32) {
        if let Some(editor) = &mut self.editor {
            ...
            editor.with_buffer_mut(|buf| {
                buf.set_size(font_sys.raw(), Some(self.bounding_box.width * scale), None);
            ...
            editor.with_buffer_mut(|buf| {
                ...
                for (line_text, line_spans, line_align) in &lines {
                    ...
                    for span in line_spans {
                        ...
                        attrs_list.add_span(
                            offset..end,
                            &span_attrs(default_attrs.clone(), span, 1.0),
                        );
                        offset = end;
                    }
                    buf.lines
    ...
    pub fn select_all(&mut self) {
        if let Some(editor) = &mut self.editor {
            ...
            let last_idx =
                editor.with_buffer(|buf| buf.lines.last().map_or(0, |l| l.text().len()));
    ...
    fn draw_bounding_box(&self, pixmap: &mut Pixmap, scale: f32) {
        let r = self.bounding_box;
        let accent = Color::from_rgba(0.0, 0.5, 1.0, 1.0);
        ...
        if let Some(path) = build_path(|pb| {
            if let Some(rect) =
                Rect::from_xywh(inset + r.x, inset + r.y, r.width - border_w, r.height - border_w)
            {
                pb.push_rect(rect);
            }
        }) {
    ...
    fn render_text_glyphs(&self, pixmap: &mut Pixmap, scale: f32) {
        ...
        if let Some((start, end)) = sel_bounds {
            ...
            editor.with_buffer(|buffer| {
                for run in buffer.layout_runs() {
                    if let Some((x_start, x_width)) = run.highlight(start, end) {
                        let rx = origin.x + x_start * inv;
                        let ry = origin.y + run.line_top * inv;
                        let rw = x_width * inv;
                        let rh = run.line_height * inv;
                        if let Some(path) = build_path(|pb| {
                            if let Some(rect) = Rect::from_xywh(rx, ry, rw, rh) {
                                pb.push_rect(rect);
                            }
                        }) {
                            fill_on_pixmap(pixmap, &path, sel_color);
                        }
                    }
                }
            });
        ...
        editor.with_buffer(|buffer| {
            for run in buffer.layout_runs() {
                ...
                for glyph in run.glyphs {
                    ...
                    let glyph_color = span_attrs.color_opt.unwrap_or(fallback_color);
                ...
                while span_start < run.glyphs.len() {
                    ...
                    while span_end < run.glyphs.len() {
                        if attrs_list.get_span(run.glyphs[span_end].start) != a {
                            break;
                        }
                        span_end += 1;
                    }
                    ...
                    if a.metadata == 1 {
                        ...
                        let baseline_offset =
                            run.line_height - span_fs * LINE_HEIGHT_FACTOR;
                        let uy = origin.y
                            + (run.line_top + baseline_offset + span_fs * LINE_HEIGHT_FACTOR)
                                * inv
                            - 1.0 / scale;
            ...
            && let Some((cx, cy)) = editor.cursor_position()
        {
            ...
            let cursor_w = 1.0 / scale;

            if let Some(path) = build_path(|pb| {
                if let Some(rect) = Rect::from_xywh(cursor_x, cursor_y, cursor_w, cursor_h) {
                    pb.push_rect(rect);
                }
            }) {
                fill_on_pixmap(pixmap, &path, self.color);
            }
        }
    }
}
...
impl ToolOperation for TextPreview {
    fn render(&self, pixmap: &mut Pixmap, _image_size: Size, scale: f32) {
        ...
        if matches!(self.state, TextEditState::Editing | TextEditState::Resizing) {
            self.draw_bounding_box(pixmap, scale);
        }
        ...
        self.render_text_glyphs(pixmap, scale);
    }
    ...
    fn on_press(&mut self, point: Point, _image_size: Size) -> mouse::Interaction {
        match self.state {
            ...
            TextEditState::Editing => {
                ...
                match handle {
                    TextDragHandle::None => mouse::Interaction::default(),
                    TextDragHandle::Move => {
                        self.blink_start.set(Instant::now());
                        let scale = self.last_scale.get();
...
fn draw_handle_pixmap(
    ...
    let rx = center.x - w * anchor_x;
    let ry = center.y - h * anchor_y;
    if let Some(path) = build_path(|pb| {
        if let Some(rect) = Rect::from_xywh(rx, ry, w, h) {
            pb.push_rect(rect);
        }
    }) {
