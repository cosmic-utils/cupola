...
pub fn fill_on_image(image: &mut DynamicImage, path: &Path, color: Color) {
    ...
    overlay.fill_path(path, &paint, FillRule::Winding, Transform::identity(), None);
    blend_overlay(rgba, &overlay);
}

/// Stroke a path onto a Pixmap overlay (for display rendering).
pub fn stroke_on_pixmap(
    pixmap: &mut Pixmap,
    path: &Path,
    color: Color,
    width: f32,
...
/// Fill a path onto a Pixmap overlay (for display rendering).
pub fn fill_on_pixmap(pixmap: &mut Pixmap, path: &Path, color: Color) {
    let mut paint = Paint::default();
    paint.set_color_rgba8(
        (color.r * 255.0) as u8,
        (color.g * 255.0) as u8,
