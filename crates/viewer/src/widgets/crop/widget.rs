...
/// - Selection mode: Shows full image with crop overlay and handles
/// - Preview mode: Shows the cropped result (when preview_handle is provided)
pub struct CropWidget {
    /// The original image handle to render
    handle: Handle,
    img_width: u32,
    img_height: u32,
    selection: CropSelection,
    /// Preview handle showing the cropped result (after Apply is clicked)
...
impl CropWidget {
    ...
    /// Set the preview handle to show the cropped result
    pub fn preview(mut self, handle: Option<Handle>, width: u32, height: u32) -> Self {
    ...
    /// Check if widget is in preview mode
    fn is_preview_mode(&self) -> bool {
        self.preview_handle.is_some()
    }
    ...
    fn calculate_preview_rect(&self, bounds: Rectangle) -> (Rectangle, f32) {
        let scale_x = bounds.width / self.preview_width as f32;
        let scale_y = bounds.height / self.preview_height as f32;
        let scale = scale_x.min(scale_y).min(1.0); // Don't upscale

        let img_w = self.preview_width as f32 * scale;
        let img_h = self.preview_height as f32 * scale;
    ...
    fn hit_test_handle(&self, img_rect: &Rectangle, scale: f32, point: Point) -> DragHandle {
        ...
        let corners = [
            (
                self.image_to_screen(img_rect, scale, rx, ry),
                DragHandle::TopLeft,
            ),
            (
                self.image_to_screen(img_rect, scale, rx + rw, ry),
                DragHandle::TopRight,
            ),
            (
                self.image_to_screen(img_rect, scale, rx, ry + rh),
                DragHandle::BottomLeft,
            ),
            (
                self.image_to_screen(img_rect, scale, rx + rw, ry + rh),
        ...
        let edges = [
            (
                self.image_to_screen(img_rect, scale, rx + rw / 2.0, ry),
                DragHandle::Top,
            ),
            (
                self.image_to_screen(img_rect, scale, rx + rw / 2.0, ry + rh),
                DragHandle::Bottom,
            ),
            (
                self.image_to_screen(img_rect, scale, rx, ry + rh / 2.0),
                DragHandle::Left,
            ),
            (
                self.image_to_screen(img_rect, scale, rx + rw, ry + rh / 2.0),
                DragHandle::Right,
            ),
        ];
    ...
    fn draw_border(&self, renderer: &mut Renderer, img_rect: Rectangle, scale: f32) {
        ...
        renderer.fill_quad(
            Quad {
                bounds: Rectangle::new(Point::new(sel_x, sel_y), Size::new(sel_w, BORDER_WIDTH)),
                ..Quad::default()
            },
            BORDER_COLOR,
        );
        ...
        renderer.fill_quad(
            Quad {
                bounds: Rectangle::new(Point::new(sel_x, sel_y), Size::new(BORDER_WIDTH, sel_h)),
                ..Quad::default()
            },
            BORDER_COLOR,
        );
...
impl Widget<Message, cosmic::Theme, Renderer> for CropWidget {
    ...
    fn draw(
        ...
        // If in preview mode, show the cropped image
        if let Some(ref preview_handle) = self.preview_handle {
            let (img_rect, _scale) = self.calculate_preview_rect(bounds);
        ...
        // Otherwise show original with crop overlay
    ...
    fn on_event(
        ...
        // In preview mode, don't handle any events (read-only)
